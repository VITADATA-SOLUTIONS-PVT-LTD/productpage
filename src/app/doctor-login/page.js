...
export default function DoctorLoginPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!data?.roles?.some((role) => doctorRoles.includes(role))) {
                throw new Error('This account does not have doctor access');
            }

            localStorage.setItem('doctorToken', data.accessToken);
            localStorage.setItem('doctorRoles', JSON.stringify(data.roles));
            localStorage.setItem('doctorUser', JSON.stringify(data.user));
            document.cookie = `doctor_token=${encodeURIComponent(data.accessToken)}; path=/; max-age=28800; samesite=lax`;
    ...
    return (
        <div className="min-h-screen bg-[#F9F9F9] flex flex-col items-center justify-center font-sans text-[#3D2010] relative overflow-hidden">
            ...
            <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(217,119,87,0.08)] w-full max-w-[320px] sm:max-w-90 p-5 sm:p-7 flex flex-col items-center mx-4 border border-[rgba(255,204,172,0.3)] relative z-10 animate-fade-up">
                ...
                <p className="text-[#6B7280] text-[13px] sm:text-[14px] mb-3">Sign in to access your clinical workspace</p>
